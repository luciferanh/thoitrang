...
$(function(){
	$('.msw_ulright li').hover(function() {
		$(this).addClass('rli');
		$('.mid_down').children('div').eq($(this).index()).addClass('show');
		$('.max_down').children('div').eq($(this).index()).addClass('show');
	}, function() {
		$(this).removeClass('rli');
		$('.mid_down').children('div').eq($(this).index()).removeClass('show');
		$('.max_down').children('div').eq($(this).index()).removeClass('show');
	});
	$('.mid_down>div').hover(function() {
		$(this).addClass('show');
	}, function() {
		$(this).removeClass('show');
	});
	$('.max_down>div').hover(function() {
		$(this).addClass('show');
	}, function() {
		$(this).removeClass('show');
	});

	var h=$('.nav').offset().top-$('header').height();
	...
	$('.showall button').click(function(event) {
		$('.msw_row .all').removeClass('alldown');
	});

	$('.msw_fluid .msw_max').hover(function() {
		$(this).children('.max_hover').addClass('show');
	}, function() {
		$(this).children('.max_hover').removeClass('show');
	});

	$('.msw_row .max_hover span').click(function(event) {
		$(this).addClass('like');
	});
	$('.msw_row .msw_mid').click(function(event) {
		return false;
	});
	$('.msw_row .msw_mid span').click(function(event) {
		$(this).addClass('like');
	});
	$('.msw_row .msw_min').click(function(event) {
		return false;
	});
	$('.msw_row .msw_min span').click(function(event) {
		$(this).addClass('like');
	});

	$('.msw_fluid .showall button').click(function(event) {
		$('.msw_fluid .moredown').show();
		$('.msw_fluid .showall').addClass('alldown');

		$('.msw_fluid .alldown button').click(function(event) {
		$('.msw_fluid .moredown1').show();
		$(this).hide();
		});
	});
	...
	$('.showall span').click(function(event) {
		$('html,body').animate({
			'scrollTop': '0'
	...
	$('.mid_sel .sel_one li,.max_down .mtb li').click(function(event) {
		$(this).css('color', '#999');
		$(this).children('input').eq(0).attr('checked', 'true');
	});

	$('.mid_sel>ul>li').click(function(event) {
		$(this).children('div').slideDown();
		$(this).children('.pp').addClass('hide');
		$(this).siblings('li').children('div').slideUp();
		$(this).siblings('li').children('.pp').removeClass('hide');
	});
